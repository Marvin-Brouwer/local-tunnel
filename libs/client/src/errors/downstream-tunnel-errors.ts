...
import { type SocketError, cleanSocketError, isSocketError } from './socket-error';
...
export class UnknownDownstreamTunnelError extends LocalTunnelError {
	...
	public get reason() {
		if (isSocketError(this.innerError)) { return this.innerError.code; }
	...
	constructor(tunnelConfig: TunnelConfig, private readonly innerError: SocketError | Error) {
		...
		this.message = `The connection to downstream tunnel ${format.localAddress(tunnelConfig)} `
