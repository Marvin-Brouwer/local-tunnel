...
import { type SocketError, cleanSocketError } from './socket-error';
import { format } from '../logger';

/**
 * Base error indicating something failed in the connection to the `proxy` tunnel connection.
 * Meaning, the the `@local-tunnel/client` host, responsible liaising between
 * the `upstream` and `downstream` connection.
 *
 ...
 * https://github.com/Marvin-Brouwer/local-tunnel/issues/new?assignees=&labels=bug&template=socket-error.md&title=}
 */
export class ProxyTunnelRejectedError extends LocalTunnelError {
	...
	constructor(address: AddressInfo, private readonly socketError: SocketError) {
		...
		this.message = `The proxy tunnel ${format.address(address)} rejected the connection. \n`
            + 'Check your firewall for outbound rules to the tunnel '
			+ `or inbound rules for port ${address.port}`;
	}
}
...
export class UnknownProxyTunnelError extends LocalTunnelError {
	...
	constructor(address: AddressInfo, private readonly socketError: SocketError) {
		...
		this.socketError = cleanSocketError(socketError);
		this.message = `The connection to Proxy tunnel ${format.address(address)} `
