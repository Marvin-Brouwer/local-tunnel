...
import { createLocalTunnel, type CertificateConfig } from '@local-tunnel/client';
import { TunnelConfig } from "@local-tunnel/client/dist/types/client/client-config";
import cp from 'node:child_process'

export const applyOptions = (...applications: ((command: Command) => Command)[]) => (command: Command) => {
    return applications.reduce((prev, application) => {
...
export const applyHttpsOptions = (command: Command) => command
  .option<CertificateConfig>('--local-cert "<pem-path> <key-path> [ca-path]"', 'Path to certificate files for local HTTPS server', validateCertificatePaths)
