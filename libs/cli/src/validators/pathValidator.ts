...
function validatePath(name: string, value: string) {
    ...
    if (!fileStats.isFile()) throw new InvalidArgumentError(`Path for ${name} "${absolutePath}" does not resolve to a file.`);
...
export function validateCertificatePaths(value: string): CertificateConfig {
    ...
    if (values.length === 1)
        throw new InvalidArgumentError(`Missing 'key-path' in arguments.`)
    if (values.length > 3)
        throw new InvalidArgumentError(`Too many arguments.`)
